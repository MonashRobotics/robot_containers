...
def create_container(image_name: str):
    ...
    - GUI application support is enabled by setting the DISPLAY environment variable, and sharing the .X11-unix socket as a volume.
    - External device support is enabled by sharing /dev as a volume.
    - Full networking support is enabled by setting network mode to 'host' and enabling the 'privileged' flag.
    - Loading of kernel modules from inside the container is enabled by sharing /lib/modules as a volume.
    - Realtime scheduling support is enabled by setting ulimits
    - The ROS distro is sourced by the ros_entrypoint.sh script supplied in the upstream docker images.
    - The current working directory is shared as a volume inside the container.
    - The container is started in the background, so that we can later attach to it from multiple terminals using 'docker exec'.
    """
...
def attach_to_container(container_name: str):
    """ Attach the terminal to an existing container, starting it if it is currently in stopped state. """
...
def container_exists(container_name: str) -> bool:
    """ Check if an image with the specified name has previously been created. It can be in stopped or running state. """
